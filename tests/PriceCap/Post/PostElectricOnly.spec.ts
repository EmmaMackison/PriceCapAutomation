...
test('DualFuel test', async ({ page }) => {
    ...
    const dualFuelBucket = parse(fs.readFileSync("src/testdata/testbuckets/Post/Simpler Energy - Multi - Elec Only - ODP - Post.csv"), {
        columns: true,
        ...
        if (zoneBasedPriceData.length) {
            ...
            if (!eleTariffName.includes('Fixed')) { //Removing account from calculation if customers currnt price is fixed
                ...
                cheapestTariffs.forEach((element) => {
                    ...
                    if (cheapestOverallEle === "1 Year Fixed + Boiler Cover - Domestic Economy") { cheapestOverallEle = "1 Year Fixed + Boiler Cover - Economy 7"; }
                });
                if (overallChecker) {
                    //First Logic to find out Extended Fixed E7 /Domestic Economy
    ...
    if (newDualFuelBucketData.length) {
        ...
        fs.writeFile('CSV Output/Multi DD Elec Only POST.csv', csvFromArrayOfObjects, err => {
            if (err) {
                console.log(18, err);
            }
