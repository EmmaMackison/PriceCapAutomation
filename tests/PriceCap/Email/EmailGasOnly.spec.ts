...
test('DualFuel test', async ({ page }) => {
    ...
    const dualFuelBucket = parse(fs.readFileSync("src/testdata/testbuckets/Email/Simpler Energy - Gas Only - ODP - Email.csv"), {
        columns: true,
    ...
    if (newDualFuelBucketData.length) {
        ...
        fs.writeFile('CSV Output/ODP Gas Only Email.csv', csvFromArrayOfObjects, err => {
            if (err) {
                console.log(18, err);
            }
