...
test('DualFuel test', async ({ page }) => {
    ...
    const dualFuelBucket = parse(fs.readFileSync("src/testdata/testbuckets/Email/Simpler Energy - Multi - Dual Fuel - ODP - Email.csv"), {
        columns: true,
    ...
    if (newDualFuelBucketData.length) {
        ...
        fs.writeFile('CSV Output/Multi ODP DF EMAIL.csv', csvFromArrayOfObjects, err => {
            if (err) {
                console.log(18, err);
            }
