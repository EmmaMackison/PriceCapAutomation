...
test('DualFuel test', async ({ page }) => {
    ...
    const dualFuelBucket = parse(fs.readFileSync("src/testdata/testbuckets/Email/Pay As You Go - Elec Only - Post.csv"), {
        columns: true,
    ...
    if (newDualFuelBucketData.length) {
        ...
        fs.writeFile('CSV Output/PAYG Elec Only POST.csv', csvFromArrayOfObjects, err => {
            if (err) {
                console.log(18, err);
            }
